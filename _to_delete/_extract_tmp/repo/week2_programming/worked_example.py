"""
Week 2 worked example - built live, in five stages.

The question: at this count site, what does an average weekday look like, and
when is the peak?
...
Each STAGE below is a step we take together in the session. Every stage runs on
its own - you should be able to comment out the later ones and still get output.

Instructor: stages 1-3 are the loop version. Stage 4 turns it into a function.
Stage 5 does the whole thing again in three lines of NumPy - that contrast is
the point of the session, and it only works because they wrote the loop first.
"""
...
# Check stage 4 agrees with stage 3. If it doesn't, one of them is wrong, and
# we would rather find out now than in the figure.
