...
You do not have to use this. If you would rather start from an empty file,
do so. But the function names and the shape of the pipeline here are the ones
we will be looking for, and each docstring says what its piece needs to do.

Run it:

    python starter/corridor.py

At the moment it loads nothing and stops. That is the honest starting point.

For the week 5 extension, `fetch_external.py` in this folder is a complete
worked example of pulling a national dataset, and `../data_sources.md` is the
catalogue of sources.
"""

import os

# You will probably want pandas. You were not taught it - that is the point of
# week 3. Ask the assistant, then check what it gives you.
...
def clean(arrivals):
    ...
    Before you write this function, go and look at the data. Open the CSV.
    Sort it. Count things. You are looking for at least five problems, and you
    will not find them by guessing.
...
def main():
    print("Data folder:", os.path.relpath(DATA, HERE))
    print()
    print("Nothing is implemented yet. Start with load_raw(), then go and")
    print("look at what you loaded before you write clean().")
